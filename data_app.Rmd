...
model_fit <- survfit(Surv(Time, Event) ~ APOE, data = tte_data)
ggsurvplot(model_fit)
```

# Check mediation analysis using the mean
```{r}
library(mediation)
library(survival)
...
set.seed(123)
mod.y <- survreg(Surv(Time, Event) ~ CSFABETA + APOE + SEX + EDUC + age_baseline, data = new_tte_data,
                 dist = "weibull")
